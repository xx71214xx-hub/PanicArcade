...
(function() {
    ...
    const coinsManager = {
        ...
        addCoins: async function(amount, reason = 'game_reward') {
            ...
            // 🛠️ الإصلاح: الإضافة تتم على السيرفر أولاً لضمان الأمان ومكافحة الغش
        ...
        deductCoins: async function(amount, reason = 'game_expense') {
            ...
            // 🛠️ الإصلاح الجوهري: نطلب الخصم من السيرفر أولاً. إذا رفض أو واجه خطأ، تعيد الدالة false فوراً ولا يفتح اللعب مجاناً.
            const { data: success, error } = await supabase.rpc('process_transaction', {
                p_tg_id: currentUser.tg_id, // تمرير معرف اللاعب للسيرفر
                p_amount: -amount,
                p_type: reason
            ...
            // الخصم المحلي وتحديث الواجهة لا يحدث إلا بعد موافقة السيرفر الصارمة
            coins -= amount;
            updateCoinsUI();
            return true;
        }
