...
(function() {
    ...
    const coinsManager = {
        ...
        addCoins: async function(amount, reason = 'game_reward') {
            ...
            // 🛠️ الإصلاح الحقيقي: تحويل المعرف الحقيقي الرقمي إلى نص (.toString()) ليتطابق مع بارامتر السيرفر المتوقع p_tg_id text
            const { data: success, error } = await supabase.rpc('process_transaction', {
                p_tg_id: currentUser.tg_id.toString(), // تمرير المعرف كنص متوافق مع السيرفر بالملي
                p_amount: amount,
                p_type: reason
            });
        ...
        deductCoins: async function(amount, reason = 'game_expense') {
            ...
            // 🛠️ الإصلاح الحقيقي: تحويل المعرف الفعلي إلى نص (.toString()) قبل الخصم، لمنع الرفض العشوائي واللعب المجاني
            const { data: success, error } = await supabase.rpc('process_transaction', {
                p_tg_id: currentUser.tg_id.toString(), // تمرير المعرف كنص متوافق مع السيرفر بالملي
                p_amount: -amount,
                p_type: reason
            ...
            if (error || success === false) {
                console.error("❌ فشلت عملية الخصم من السيرفر أو تم رفضها.");
                return false;
            }
    ...
    async function handleDailyReward() {
        ...
        // 🛠️ التوافق مع مكافأة اليومية أيضاً وتحويل المعرف لنص لتجنب تعليق الكاونت داون
        ...
        rewardBtns.forEach(btn => {
            if (btn) {
                ...
                newBtn.addEventListener("click", async () => {
                    ...
                    // 🛠️ تأمين زر الاستلام اليومي بالنص المتوافق
                    const { data: result, error } = await supabase.rpc('claim_daily_reward', {
                        p_tg_id: currentUser.tg_id.toString() 
                    });
