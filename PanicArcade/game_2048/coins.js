...
(function() {
    ...
    const coinsManager = {
        ...
        addCoins: async function(amount, reason = 'game_reward') {
            ...
            // نرسل القيمة كرقم خالص متوافق مع بنية الجدول الأساسية في الـ RPC
            const { data: success, error } = await supabase.rpc('process_transaction', {
                p_tg_id: Number(currentUser.tg_id), 
        ...
        deductCoins: async function(amount, reason = 'game_expense') {
            ...
            // الحسم الصارم: نرسل المعرف كرقم وننتظر تأكيد السيرفر بإرجاع true حصراً
            const { data: success, error } = await supabase.rpc('process_transaction', {
                p_tg_id: Number(currentUser.tg_id), 
                p_amount: -Number(amount),
                p_type: reason
            ...
            // إذا أرجع السيرفر خطأ أو لم يرجع true، نرفض العملية فوراً ولا تفتح اللعبة مجاناً
            if (error || success !== true) {
                console.error("❌ فشلت عملية الخصم من السيرفر أو تم رفضها.");
                alert("عذراً، فشل تأمين العملية عبر السيرفر.");
                return false;
            }
